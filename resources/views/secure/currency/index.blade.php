...
@canany(['add currency', 'edit currency'])
{{-- Modal --}}
        ...
        <div class="modal-content">
            ...
            <form class="form" id="form">
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3">
                        <label for="amount_in_dollars" class="form-label">Amount in Dollars (1 unit = ? USD)</label>
                        <input type="number" step="0.0001" class="form-control" name="amount_in_dollars" id="amount_in_dollars" required>
                    </div>
                </div>
...
<script @cspNonce>
    ...
    $(document).ready(function() {
        ...
        dataTable = $('#datatable').DataTable({
            ...
            scrollX: true,
            ajax: {
                url: "{{ route('currencies.fetch-for-datatable') }}",
                type: "POST",
                data: function(d) {
    ...
    $(document).ready(function() {
        ...
        $('#form').on('submit', function(event) {
            ...
            if (operationType == 'EDIT') {
                url = "{{ route('currencies.update', ':id') }}".replace(':id', recordId);
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-edit', function() {
            ...
            if (id) {
                operationType = "EDIT";
                $.ajax({
                    url: "{{ route('currencies.fetch-one', ':id') }}".replace(':id', id),
                    type: 'GET',
                    headers: {
                    ...
                    success: function(response) {
                        if (response.success == true) {
                            ...
                            $('#amount_in_dollars').val(data.amount_in_dollars);
            ...
            if (id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('currencies.destroy', ':id') }}".replace(':id', id),
                            type: "POST",
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
