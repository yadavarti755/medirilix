...
@canany(['add intended use', 'edit intended use'])
{{-- Modal --}}
...
<script @cspNonce>
    ...
    $(document).ready(function() {
        ...
        dataTable = $('#datatable').DataTable({
            ...
            scrollX: true,
            ajax: {
                url: "{{ route('intended-uses.fetch-for-datatable') }}",
                type: "POST",
                data: function(d) {
    ...
    $(document).ready(function() {
        ...
        $('#form').on('submit', function(event) {
            ...
            if (operationType == 'EDIT') {
                url = "{{ route('intended-uses.update', ':id') }}".replace(':id', recordId);
            } else if (operationType == 'ADD') {
                url = "{{ route('intended-uses.store') }}";
            } else {
                return false;
            }
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-edit', function() {
            ...
            if (id) {
                operationType = "EDIT";
                $.ajax({
                    url: "{{ route('intended-uses.fetch-one', ':id') }}".replace(':id', id),
                    type: 'GET',
                    headers: {
            ...
            if (id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('intended-uses.destroy', ':id') }}".replace(':id', id),
                            type: "POST",
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
