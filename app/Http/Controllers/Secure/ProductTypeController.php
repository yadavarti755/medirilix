...
class ProductTypeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('secure.product_types.index', compact('pageTitle'));
    }

    // Function to get all product types in for data table;
    public function fetchForDatatable(Request $request)
    {
        if ($request->ajax()) {
            $productTypes = $this->service->findAll();
            return DataTables::of($productTypes)
                ->addColumn('action', function ($productType) {
                    $button = '';
                    if (auth()->user()->can('edit product type')) {
                        $button .= '<button type="button" data-id="' . $productType->id . '" class="btn btn-sm btn-warning btn-edit" title="Edit"><i class="fa fa-edit"></i></button> ';
                    }

                    if (auth()->user()->can('delete product type')) {
                        $button .= '<button class="btn btn-sm btn-danger btn-delete" data-id="' . $productType->id . '" title="Delete">
    ...
    public function update(StoreProductTypeRequest $request, ProductType $productType)
    {
        try {
            ...
            $result = $this->service->update($dto, $productType->id);
    ...
    public function destroy(string $id)
    {
        try {
            ...
            Log::error('ProductType: Deletion failed: ' . $e->getMessage());
